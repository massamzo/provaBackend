	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(type_req.equals(typesOfRequests[0])) {
			...
			try {
				...
				torneo = new Torneo(sessionID, data, nome_evento, orario);
				Torneo torneoDaAbbandonare = torneo.getTorneo();
				
				User userDaTogliere = torneo.getUserKnowingID(); // prendo l'utente utilizzando sessionID
				
				torneoDaAbbandonare.abbandonaTorneo(userDaTogliere.getEmail());
